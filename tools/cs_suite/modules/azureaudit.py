...
log = logger.get()
baccount_name = subprocess.check_output(
    ['az account list --all --query [*].[tenantId,isDefault] --output tsv | grep True | cut -f1'], shell=True).strip()
account_name = utils.bytes_to_str_noquotes(baccount_name)
        ...
        with open(file, 'r') as json_data:
            for line in json_data:
                ...
                for k in final['data']:
                    if k['type'] == 'WARNING':
                        f.write(
                            '<p><span style="color:red">Warning: </span>%s</p>\n' % (k['data']))
                    else:
                        f.write('<p>%s<p>\n' % (k['data']))
                f.write('</div>')
                f.write('</div>')
...
def no_guest_user():
    ...
    guest_user_list = subprocess.check_output(
        ['az ad user list --query "[?additionalProperties.userType==\'Guest\']"'], shell=True)
    result = {}
...
def custom_owner_role():
    ...
    definition_list = subprocess.check_output(
        ['az role definition list'], shell=True)
    print(definition_list)


def automatic_provising_agent():
    print("2.2: Checking for Automatic Provising Agent\n\n")
    agent_provising = subprocess.check_output(
...
def system_update():
    ...
    system_update = subprocess.check_output(
...
def security_configuration():
    ...
    sec_config = subprocess.check_output(
...
def endpoint_protection():
    ...
    end_protect = subprocess.check_output(
...
def disk_encryption():
    ...
    encrypt_disk = subprocess.check_output(
...
def adaptive_application_control():
    ...
    app_control = subprocess.check_output(
...
def sql_auditing():
    ...
    sql_detect = subprocess.check_output(
...
def sql_encryption():
    ...
    encrypt_sql = subprocess.check_output(
...
def security_email_set():
    ...
    email_config = subprocess.check_output(
...
def security_contact_phone():
    ...
    phone_config = subprocess.check_output(
...
def email_for_alert():
    ...
    alert_email = subprocess.check_output(
...
def email_for_subs_owners():
    ...
    email_owner = subprocess.check_output(
...
def secure_transfer():
    ...
    https_enabled = subprocess.check_output(
        ['az storage account list --query \[*\].\[name,enableHttpsTrafficOnly,primaryLocation\] --output tsv'], shell=True).strip()
    result = {}
...
def storage_service_encryption():
    ...
    storage_encryption = subprocess.check_output(
        ['az storage account list --query \[*\].\[name,encryption.services.blob.enabled,primaryLocation\] --output tsv'], shell=True).strip()
    result = {}
...
def access_key_rotation():
    resource_ids = subprocess.check_output(
        ['az storage account list --query [*].[id] --output tsv'], shell=True)
    ids = []
    for resource in resource_ids.splitlines():
        ids.append(resource)

    for id in set(ids):
        time_stmp = subprocess.check_output(
            ['az monitor activity-log list --resource-id %s --query [*].[eventTimestamp,resourceGroup] --output tsv' % id], shell=True)
        if time_stmp:
...
def encrption_file_service():
    ...
    file_encryption = subprocess.check_output(
        ['az storage account list --query \[*\].\[name,encryption.services.file.enabled,primaryLocation\] --output tsv'], shell=True).strip()
    result = {}
...
def log_profile():
    ...
    check_profile = subprocess.check_output(
        ['az monitor log-profiles list --query [*].[id,name]'], shell=True).strip()
    result = {}
    j_res = {}
    data = []
...
def log_retention():
    ...
    retention_policy = subprocess.check_output(
        ['az monitor log-profiles list --query [*].retentionPolicy.enabled --output tsv'], shell=True).strip()
    result = {}
    j_res = {}
    data = []
    ...
    if retention_policy in ['false', '']:
        ...
        days = subprocess.check_output(
            ['az monitor log-profiles list --query [*].retentionPolicy.days --output tsv'], shell=True).strip()
        if days == b'': 
            j_res['type'] = 'INFO'
            j_res['value'] = 'The LOG RETENTION policy not found'
        elif int(days) < 365:
            j_res['type'] = 'WARNING'
...
def alert_for_create_policy():
    ...
    resource_groups = subprocess.check_output(
        ['az monitor activity-log alert list --query \[*\].\[resourceGroup\] --output tsv'], shell=True).strip()
    result = {}
    ...
    for resource_group in resource_groups.splitlines():
        ...
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Authorization/policyAssignments/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_group_create_network():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Network/networkSecurityGroups/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_group_network_delete():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Network/networkSecurityGroups/delete"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_rule_network_create():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Network/securityRules/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_create_security():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Security/securitySolutions/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_delete_security():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Security/securitySolutions/delete"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_create_sql_rule():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Sql/servers/firewallRules/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_delete_sql_rule():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Sql/servers/firewallRules/delete"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def alert_update_security_policy():
    ...
    for resource_group in resource_groups.splitlines():
        ...
        j_res['category'] = 'management'
        check = subprocess.check_output(
            ['az monitor activity-log alert list --resource-group %s --query [*].condition | jq \'.|.[].allOf[] | select(.equals | contains("Microsoft.Security/policies/write"))\'' % resource_group], shell=True).strip()
        if check == "":
...
def rdp_public():
    ...
    network_list = subprocess.check_output(
        [' az network nsg list --query [*].name --output tsv'], shell=True).strip()
    result = {}
    data = []
    result['check'] = 'PUBLIC_RDP_ACCESS'
    for bnetwork_group in network_list.splitlines():
        network_group = utils.bytes_to_str(bnetwork_group)
        flag = 0
...
def ssh_public():
    ...
    for bnetwork_group in network_list.splitlines():
        ...
        lines = subprocess.check_output(
            ['az network nsg list --query "[?name==%s].[securityRules][][].[access,destinationPortRange,direction,protocol,sourceAddressPrefix]"  --output tsv' % network_group], shell=True).strip()
        for line in lines.splitlines():
            access_type, port, direction, protocol, source = line.split()
            j_res = {}
            j_res['check_no'] = '6.2'
...
def network_watcher():
    ...
    check = subprocess.check_output(
        ['az network watcher list'], shell=True).strip()
    result = {}
    data = []
...
def vm_agent():
    ...
    lines = subprocess.check_output(
        ['az vm list --query [*].[resourceGroup,name] --output tsv'], shell=True).strip()
    result = {}
    result['check'] = 'VM_AGENT'
    data = []
    for line in lines.splitlines():
        bresource_group, bname = line.split()
        resource_group = utils.bytes_to_str(bresource_group)
        name = utils.bytes_to_str(bname)
        check = subprocess.check_output(
            ['az vm show -g %s -n %s --query resources[*].[virtualMachineExtensionType,provisioningState] --output tsv' % (resource_group, name)], shell=True).strip()
        j_res = {}
        j_res['check_no'] = '7.1'
        j_res['level'] = 'INFO'
        j_res['score'] = 'Scored'
        ...
        if check == '':
            j_res['type'] = 'WARNING'
            j_res['value'] = 'The VM %s does not have virtual agent enabled' % (
                name)
        else:
            list = check.split()
            if not list:
                j_res['type'] = 'PASS'
                j_res['value'] = 'The VM %s does have virtual agent enabled' % (
                    name)
        data.append(j_res)
        log_data = dict()
        log_data = j_res
...
def vm_os_disk():
    ...
    for line in lines.splitlines():
        ...
        if check == "WARNING: Azure Disk Encryption is not enabled":
            j_res['type'] = 'WARNING'
            j_res['value'] = 'The VM %s does not have OS DISK ENCRYPTION enabled' % (
                name)
        else:
            j_res['type'] = 'PASS'
            j_res['value'] = 'The VM %s does have OS DISK ENCRYPTION enabled' % (
                name)
        data.append(j_res)
        log_data = dict()
        log_data = j_res
...
def vm_data_disk():
    ...
    for line in lines.splitlines():
        ...
        resource_group = utils.bytes_to_str(bresource_group)
        name = utils.bytes_to_str(bname)
        check = subprocess.check_output(['az vm encryption show --resource-group %s --name %s  --query dataDisk  --output tsv' % (
            resource_group, name)], shell=True, stderr=subprocess.STDOUT).strip()
        j_res = {}
        j_res['check_no'] = '7.3'
        j_res['level'] = 'INFO'
        j_res['score'] = 'Scored'
        ...
        if check == "WARNING: Azure Disk Encryption is not enabled":
            j_res['type'] = 'WARNING'
            j_res['value'] = 'The VM %s does not have DATA DISK ENCRYPTION enabled' % (
                name)
        else:
            j_res['type'] = 'PASS'
            j_res['value'] = 'The VM %s does have DATA DISK ENCRYPTION enabled' % (
                name)
        data.append(j_res)
        log_data = dict()
        log_data = j_res
...
def approved_extension():
    ...
    for line in lines.splitlines():
        ...
        name = utils.bytes_to_str(bname)
        check = subprocess.check_output(
            ['az vm extension list --resource-group %s  --vm-name %s --query [*].name --output tsv' % (resource_group,  name)], shell=True).strip()
        if check != '':
    ...
    if len(extensions) == 0:
        ...
        j_res['type'] = "WARNING"
        j_res['value'] = 'Please manually check for approval for these extensions %s' % list(
            set(extensions))
    data.append(j_res)
    log_data = dict()
    log_data = j_res
...
def vault_key():
    ...
    data = []
    j_res = {}
    bcheck_vault_exists = subprocess.check_output(
        ['az keyvault list --query [*].name --output tsv'], shell=True).strip()
    check_vault_exists = utils.bytes_to_str_noquotes(bcheck_vault_exists)
    if check_vault_exists == '':
        ...
        for bvault in check_vault_exists.splitlines():
            vault = utils.bytes_to_str(bvault)
            expiry = subprocess.check_output(
                ['az keyvault key list --vault-name %s --query [*].[kid,attributes.expires] --output tsv' % (vault)], shell=True).strip()
            for exp in expiry.splitlines():
                ...
                if date == 'None':
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'No expiry date set for key : %s' % (
                        key_name)
                elif exp == '':
                    j_res['type'] = 'PASS'
                    ...
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'Access Denied could not check for expiration in vault %s' % (
                        expiry)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
...
def vault_secret():
    ...
    if check_vault_exists == '':
        ...
        for bvault in check_vault_exists.splitlines():
            vault = utils.bytes_to_str(bvault)
            expiry = subprocess.check_output(
                ['az keyvault secret list --vault-name %s --query [*].[id,attributes.expires] --output tsv' % (vault)], shell=True).strip()
            for exp in expiry.splitlines():
                key_name, date = exp.split()
                j_res = {}
                j_res['check_no'] = '8.2'
                ...
                if date == 'None':
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'No expiry date set for secret : %s' % (
                        key_name)
                elif exp == '':
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'No keys found in vault %s' % (expiry)
...
def sql_db_audit():
    ...
    check_server_exists = subprocess.check_output(
        ['az sql server list --query [*].[name,resourceGroup] --output tsv'], shell=True).strip()
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            bname, bresource_group = server.split()
            name = utils.bytes_to_str(bname)
            resource_group = utils.bytes_to_str(bresource_group)
            databases = subprocess.check_output(
                ['az sql db list --server %s --resource-group %s --query [*].name --output tsv' % (name, resource_group)], shell=True).strip()
            for database in databases.splitlines():
                ...
                audit_policy = subprocess.check_output(
                    ['az sql db audit-policy show --resource-group %s --server %s --name %s --query \'state\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if audit_policy == "Disabled":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s does not have AUDIT Policy enabled' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s does have AUDIT Policy enabled' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                threat_policy = subprocess.check_output(
                    ['az sql db threat-policy show --resource-group %s --server %s --name %s --query \'state\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if threat_policy == "Disabled":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s does not have Threat Detection enabled' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s does have Threat Detection enabled' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
    ...
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                j_res['category'] = 'database'
                threat_policy = subprocess.check_output(
                    ['az sql db threat-policy show --resource-group %s --server %s --name %s --query \'disabledAlerts\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if threat_policy != "":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has some of the alerts disabled' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s does not have alerts disabled' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
    ...
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                j_res['category'] = 'database'
                threat_policy = subprocess.check_output(
                    ['az sql db threat-policy show --resource-group %s --server %s --name %s --query \'emailAddresses\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if threat_policy == "Disabled":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has some no email set for alerts' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s has some email set for alerts' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
    ...
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                j_res['region'] = 'null'
                j_res['category'] = 'database'
                email_policy = subprocess.check_output(
                    ['az sql db threat-policy show --resource-group %s --server %s --name %s --query \'emailAccountAdmins\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if email_policy == "Disabled":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has no Admin email set for alerts' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s has some Admin email set for alerts' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                encryption = subprocess.check_output(
                    ['az sql db tde show --resource-group %s --server %s --database %s --query \'status\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if encryption == "Disabled":
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has Transparent Data Encryption disabled' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s has Transparent Data Encryption enabled' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
    ...
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                j_res['category'] = 'database'
                days = subprocess.check_output(
                    ['az sql db audit-policy show --resource-group %s --server %s --name %s --query \'retentionDays\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if int(days) <= 90:
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has AUDIT log retention policy lesser than 90 days' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s has AUDIT log retention policy greater than 90 day' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
    ...
    if not check_server_exists:
        ...
        for server in check_server_exists.splitlines():
            ...
            for database in databases.splitlines():
                ...
                j_res['category'] = 'database'
                days = subprocess.check_output(
                    ['az sql db threat-policy show --resource-group %s --server %s --name %s --query \'retentionDays\' --output tsv' % (resource_group, name, database)], shell=True).strip()
                if int(days) <= 90:
                    j_res['type'] = 'WARNING'
                    j_res['value'] = 'The SQL DB %s on server %s has THREAT log retention policy lesser than 90 days' % (
                        database, name)
                else:
                    j_res['type'] = 'PASS'
                    j_res['value'] = 'The SQL DB %s on server %s has THREAT log retention policy greater than 90 day' % (
                        database, name)
                data.append(j_res)
                log_data = dict()
                log_data = j_res
            ...
            with open(json_file, 'r') as f:
                for line in f:
                    ...
                    if j['check'] == check:
                        ...
                        data.append(j)
                dict['data'] = data
                g.write("%s\n" % (json_util.dumps(dict)))


def persis(j1, j2):
    f = open("./reports/AZURE/%s/%s/diff.json" %
             (account_name, timestmp), "a+")
    for data1 in j1['data']:
        for data2 in j2['data']:
            if data1 == data2:
                pers = json_util.dumps(data1)
                pers = json_util.loads(pers)
                if pers['type'] == 'WARNING':
                    pers['check'] = j1['check']
                    f.write("%s\n" % json_util.dumps(pers))
    persistent_json("./reports/AZURE/%s/%s/diff.json" %
...
def persistent(latest, last):
    with open("./reports/AZURE/%s/%s/diff.json" % (account_name, timestmp), "a+") as h:
        with open(latest, 'r') as f:
            for line1 in f:
                data1 = json_util.loads(line1)
                for i in data1['data']:
                    with open(last, 'r') as g:
                        for line2 in g:
                            data2 = json_util.loads(line2)
                            for k in data2['data']:
...
def persistent_files():
    ...
    if len(dirs) == 1:
        ...
        with open("./reports/AZURE/%s/%s/diff.html" % (account_name, timestmp), 'w') as f:
            f.write(
                "This is the first audit run for the account, diff will be shown in the next run")
    else:
...
def azure_audit():
    subprocess.call(
        ['mkdir', '-p', 'reports/AZURE/%s/%s/final_report' % (account_name, timestmp)])
    ...
    subprocess.check_output(
        ['rm ./reports/AZURE/%s/%s/final_report/report.html' % (account_name, timestmp)], shell=True)
    webbrowser.open('file://' + os.path.realpath('./reports/AZURE/%s/%s/final_report/report_azure.html')
                    % (account_name, timestmp))
    fin = os.path.realpath(
        './reports/AZURE/%s/%s/final_report/report_azure.html') % (account_name, timestmp)
    print('THE FINAL REPORT IS LOCATED AT --------> ' + INFO + '%s' % (fin) + CLOSE)
